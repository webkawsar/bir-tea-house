...
$status_ins = false;
$status_act = false;
$btn_text = esc_html__('Install All', 'organify');
...
$merlin_setup = get_option( 'merlin_' . organify()->get_slug() . '_completed' );
 
if( $status_ins && $status_act)
	$btn_text = esc_html__('Install & Active All', 'organify');
else if($status_ins && !$status_act)
	$btn_text = esc_html__('Install All', 'organify');
else if(!$status_ins && $status_act)
	$btn_text = esc_html__('Active All', 'organify');
?>
<main>
	...
	<div class="pxl-dashboard-wrap">
		...
		<header class="pxl-dsb-header admin-plugin">
			<div class="pxl-dsb-header-inner">
				<h4><?php esc_html_e( 'Install Plugins', 'organify' ); ?></h4>
			...
			<p><?php esc_html_e( 'Make sure to activate required plugins prior to import a demo.', 'organify' ); ?></p> 
		</header>
	        ...
	        <?php
				...
				foreach( $plugins as $plugin ) :
					...
					elseif( !is_plugin_inactive( $file_path ) ) {
						...
						$display_status = esc_html__( 'Active:', 'organify' );
					}
			?>
				<div class="pxl-col pxl-col-3">
					...
					<?php 
					$barplugin = new Organify_Admin_Plugins;
